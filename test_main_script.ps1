# Test script for language detection in the main script
# This is a simplified version of newpc.ps1 that only tests the language detection
    ...
    try {
        ...
        if ($script:Translations.ContainsKey($languageCode)) {
            ...
            if ($script:Translations.ContainsKey('en')) {
                ...
                Write-LogMessage "Language not supported, using English as fallback"
                return 'en'
            }
            else {
                Write-Host "Language not supported, using Dutch as fallback" -ForegroundColor Yellow
                Write-LogMessage "Language not supported, using Dutch as fallback"
                return 'nl'
            }
        }
    }
        ...
        return 'en' # Default to English on error for this test
    }
}
...
try {
    Clear-Host
    Write-Host "=== Testing Language Detection in Main Script ===" -ForegroundColor Cyan
